...
# Define label list
label_list = ['Completa 3+', 'Completa 2+', 'Completa 1+', 'Incompleta', 'Ausente']
...
def update_ann_image(session_state, all_points, all_labels, image):
    ...
    label_colors = {
        0: (255, 0, 0),      # Red for Completa 3+
        1: (255, 165, 0),    # Orange for Completa 2+
        2: (255, 255, 0),    # Yellow for Completa 1+
        3: (0, 255, 0),      # Green for Incompleta
        4: (0, 0, 255),      # Blue for Ausente
    }
...
def check_latest_session_log(log_path = "./her2_annotator/latest_session.log"):
    try:
...
def store_latest_session_log(file_name, log_path = "./her2_annotator/latest_session.log"):
    try:
